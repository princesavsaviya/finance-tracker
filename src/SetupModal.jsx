...
export default function SetupModal({ onComplete, initial }) {
  ...
  const [repo, setRepo] = useState(initial?.repo || 'finance-tracker-data');
    ...
    <div className="fixed inset-0 bg-zinc-950/95 backdrop-blur z-50 flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg max-w-lg w-full p-6 my-8">
        ...
        <p className="text-sm text-zinc-500 mb-5">
          Your PAT is stored only in this browser's localStorage. Sent only to api.github.com.
        </p>
        ...
        <details className="mb-5 bg-zinc-950/50 border border-zinc-800 rounded p-3 text-sm">
          ...
          <ol className="mt-3 space-y-1.5 text-zinc-400 list-decimal list-inside pl-1">
            <li>Go to <a className="text-cyan-400 hover:underline inline-flex items-center gap-0.5" href="https://github.com/settings/personal-access-tokens/new" target="_blank" rel="noopener noreferrer">github.com/settings/personal-access-tokens/new <ExternalLink size={11} /></a></li>
            <li>Token name: <span className="font-mono text-zinc-300">finance-tracker-data-rw</span></li>
